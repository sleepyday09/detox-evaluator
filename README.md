...
2. 필요하면 보존할 핵심 표현을 입력합니다.
3. SIM, 표현 중복 지표, 변경된 토큰, 숫자·부정 표현 확인 항목을 함께 살펴봅니다.
4. CSV 또는 JSON으로 결과를 저장합니다. JSON에는 모델 리비전과 설정이 포함됩니다.
5. CSV 일괄 평가에서는 `examples.csv` 양식을 사용합니다. 필수 열은 `source,candidate`, 선택 열은 `keywords`입니다. 쉼표가 들어가는 셀은 CSV 규칙에 따라 큰따옴표로 감쌉니다.
...
| BLEU | SacreBLEU, Unicode 정규식 토큰, 1~4-gram, exp smoothing, effective order, 0~100 | 원문 대비는 self-BLEU 성격의 중복 측정 |
